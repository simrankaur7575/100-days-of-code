/*
QUES - Given n points on a 2D plane, find the maximum number of points that lie on the same straight line.
*/
int gcd(int x, int y) {
...
if(N<0){ N*=-1; D*=-1; }
if((N==0)&&(D==0)) ++overlap;
else if(D==0) {++vertical;cur=(cur>=vertical)?cur:vertical;}
else{
int X=gcd(N,D);
N/=X;
D/=X;
